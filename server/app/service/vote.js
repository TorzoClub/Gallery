  ...
  class VoteService extends Service {
    ...
    async create({ gallery_id, photo_id, qq_num }) {
      return this.app.model.transaction(async transaction => {
        const gallery = await this.GalleryModel.findByPk(gallery_id, { transaction, lock: transaction.LOCK.UPDATE });
        if (!gallery) {
          throw new app.WarningError('相册不存在', 404);
        }

        const photo = await this.PhotoModel.findByPk(photo_id, { transaction, lock: transaction.LOCK.UPDATE });
        if (!photo) {
        ...
        await photo.increment('vote_count', { by: 1, transaction, lock: transaction.LOCK.UPDATE });
